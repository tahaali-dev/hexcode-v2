...
import { Search, AlertOctagon, AlertTriangle, AlertCircle, CheckCircle, ChevronDown, Info, Lock } from 'lucide-react';
import { PrimaryBtn } from '../../Components/Buttons';
import { Input } from '../../Components/ContactModal';
import { fetchAuditData } from './actions';
...
export default function AuditReportPage() {
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    const result = await fetchAuditData(passwordInput);
    ...
    return (
      <div className={styles.section}>
        <div className={styles.sectionHeader}>
          ...
          <span className={styles.sectionCount}>{list.length} ISSUES</span>
        </div>
            ...
            return (
              <div key={idx} className={`${styles.issueCard} ${isExpanded ? styles.expanded : ''}`}>
                <div className={styles.issueHeader} onClick={() => toggleIssue(id)}>
                  ...
                  <ChevronDown className={`${styles.chevron} ${isExpanded ? styles.chevronOpen : ''}`} size={20} />
                </div>

                {isExpanded && (
                  <div className={styles.issueContent}>
                    <div className={styles.impactBox}>
                      <AlertTriangle className={styles.impactIcon} size={20} />
      ...
      <DashedContainer leftBottom rightBottom>
        <div className={styles.passwordContainer}>
          <div className={styles.passwordCard}>
            ...
            <p>Please enter the password provided by HEXCODE to continue.</p>
            <form onSubmit={handlePasswordSubmit}>
              <Input
                type="password"
                placeholder="Enter access key"
                value={passwordInput}
                onChange={(e: any) => { setPasswordInput(e.target.value); setPasswordError(false); }}
              ...
              <PrimaryBtn
                ...
                margin="0"
                borderRadius="8px"
                btnContent={isAuthenticating ? "Verifying..." : "Continue"}
      ...
      <div className={styles.container}>
        <header className={styles.hero}>
          ...
          <p className={styles.heroSubtitle}>
            Comprehensive senior-level code audit aligned with the client PRD.
            40 findings across Security, Architecture, Performance, and Code Quality — prioritized by production risk and business impact.
          </p>
        </header>
        ...
        <div className={styles.statsGrid}>
          <div className={`${styles.statCard} ${styles.critical} ${severityFilter === 'CRITICAL' ? styles.activeStatCard : ''}`} onClick={() => setSeverityFilter(severityFilter === 'CRITICAL' ? 'ALL' : 'CRITICAL')}>
            <AlertOctagon className={styles.statIcon} size={32} />
            <div className={styles.statDetails}>
              <div className={styles.statNum}>{totalCritical}</div>
              <div className={styles.statLabel}>Critical</div>
            </div>
          </div>
          <div className={`${styles.statCard} ${styles.high} ${severityFilter === 'HIGH' ? styles.activeStatCard : ''}`} onClick={() => setSeverityFilter(severityFilter === 'HIGH' ? 'ALL' : 'HIGH')}>
            <AlertTriangle className={styles.statIcon} size={32} />
            <div className={styles.statDetails}>
              <div className={styles.statNum}>{totalHigh}</div>
              <div className={styles.statLabel}>High</div>
            </div>
          </div>
          <div className={`${styles.statCard} ${styles.medium} ${severityFilter === 'MEDIUM' ? styles.activeStatCard : ''}`} onClick={() => setSeverityFilter(severityFilter === 'MEDIUM' ? 'ALL' : 'MEDIUM')}>
            <AlertCircle className={styles.statIcon} size={32} />
            <div className={styles.statDetails}>
              <div className={styles.statNum}>{totalMedium}</div>
              <div className={styles.statLabel}>Medium</div>
            </div>
          </div>
          <div className={`${styles.statCard} ${styles.low} ${severityFilter === 'LOW' ? styles.activeStatCard : ''}`} onClick={() => setSeverityFilter(severityFilter === 'LOW' ? 'ALL' : 'LOW')}>
            <CheckCircle className={styles.statIcon} size={32} />
            <div className={styles.statDetails}>
        ...
        <main className={styles.main}>
          {filteredIssues.length === 0 ? (
            <div className={styles.noResults}>
              <Info className={styles.noResultsIcon} size={32} />
